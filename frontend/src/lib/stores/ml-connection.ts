...
export const isMLConnected = derived(
	{ subscribe },
	($mlConnection) => $mlConnection.isConnected
);
...
export const mlUserInfo = derived(
	{ subscribe },
	($mlConnection) => ({
...
export const mlConnectionStore = {
	 ...
	 */
	async initiateConnection(redirectUri: string, siteId: string = 'MLA'): Promise<MLOAuthInitiateResponse> {
		...
		try {
			...
			if (response.success) {
				...
				update(currentState => ({
					...currentState,
					isConnected: true,
					connectionHealth: response.connection_health as 'healthy' | 'expired' | 'invalid' | 'disconnected',
