<script lang="ts">
	...
	import { mlConnectionStore, isMLConnected, mlConnectionHealth, mlUserInfo } from '$lib/stores/ml-connection';
	import { CONNECTION_HEALTH_LABELS, CONNECTION_HEALTH_COLORS, ML_SITES } from '$lib/types/ml-connection';
	import type { MLConnectionState } from '$lib/types/ml-connection';
	...
	async function refreshStatus() {
		...
		try {
			...
			console.error('Failed to refresh ML connection status:', error);
		} finally {
			isRefreshing = false;
		}
	}
	...
	async function handleDisconnect() {
		if (confirm('Are you sure you want to disconnect your MercadoLibre account?')) {
			try {
	...
	<div class="ml-status-compact">
		...
		{#if showActions && !connectionState.isLoading}
			<div class="compact-actions">
				{#if isConnected}
					{#if needsAttention}
						<button class="action-button warning" on:click={handleConnect}>
							Reconnect
						</button>
				...
				<div class="connected-state">
					...
					{#if showActions}
						<div class="status-actions">
							...
							{:else}
								{#if connectionState.shouldRefresh}
									<button 
										class="action-button secondary"
