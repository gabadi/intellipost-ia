...
describe('MLConnectionModal', () => {
	...
	it('should allow site selection and show selected site', async () => {
		const { component } = render(MLConnectionModal, {
		...
		expect(component.selectedSiteId).toBe('MBL');
	});
	...
	it('should disable buttons during connection process', async () => {
		const mockInitiateConnection = vi.fn().mockImplementation(() => 
	...
	it('should show connecting state with proper messaging', async () => {
		...
		expect(screen.getByText('Remember: Only manager accounts can complete this process.')).toBeInTheDocument();
	});
