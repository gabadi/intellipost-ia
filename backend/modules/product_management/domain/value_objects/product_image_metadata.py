...
class ProductImageMetadata(BaseValueObject):
    ...
    def validate(self) -> None:
        ...
        for score_name, score_value in [
            ...
            if score_value is not None:
                if not isinstance(score_value, (int, float)):
                    errors.append(f"{score_name} must be a number, got {type(score_value).__name__}")
                elif not (self.MIN_QUALITY_SCORE <= score_value <= self.MAX_QUALITY_SCORE):
                    errors.append(
        ...
        if self.compression_ratio is not None:
            if not isinstance(self.compression_ratio, (int, float)):
                errors.append(f"compression_ratio must be a number, got {type(self.compression_ratio).__name__}")
            elif self.compression_ratio <= 0:
                errors.append(f"compression_ratio must be positive, got {self.compression_ratio}")
        ...
        for color_list, list_name in [
            ...
            for i, color in enumerate(color_list):
                if not isinstance(color, str):
                    errors.append(f"{list_name}[{i}] must be a string, got {type(color).__name__}")
        ...
        for i, obj in enumerate(self.detected_objects):
            if not isinstance(obj, dict):
                errors.append(f"detected_objects[{i}] must be a dict, got {type(obj).__name__}")
                continue
            ...
            for field_name in required_fields:
                if field_name not in obj:
                    errors.append(f"detected_objects[{i}] missing required field: {field_name}")
        ...
        for i, step in enumerate(self.processing_steps):
            if not isinstance(step, dict):
                errors.append(f"processing_steps[{i}] must be a dict, got {type(step).__name__}")
                continue
            ...
            for field_name in required_fields:
                if field_name not in step:
                    errors.append(f"processing_steps[{i}] missing required field: {field_name}")
    ...
    def validate_field(self, field_name: str, field_value: Any) -> None:
        ...
        """
        if field_name in ("quality_score", "sharpness_score", "brightness_score", "contrast_score"):
            if field_value is not None:
                if not isinstance(field_value, (int, float)):
                    raise ValueObjectValidationError(
                        f"{field_name} must be a number, got {type(field_value).__name__}"
                    )
                if not (self.MIN_QUALITY_SCORE <= field_value <= self.MAX_QUALITY_SCORE):
                    raise ValueObjectValidationError(
