...
class ContentValidationResult(BaseValueObject):
    ...
    def validate(self) -> None:
        ...
        for field_name, score in score_fields:
            if score is not None:
                if not isinstance(score, int | float):
                    ...
                    self.MIN_VALIDATION_SCORE <= score <= self.MAX_VALIDATION_SCORE
                ):
                    errors.append(
        ...
        if len(self.validation_errors) > self.MAX_ERRORS:
            errors.append(
                f"Too many validation errors: {len(self.validation_errors)} (max: {self.MAX_ERRORS})"
            )

        if len(self.validation_warnings) > self.MAX_WARNINGS:
            errors.append(
                f"Too many validation warnings: {len(self.validation_warnings)} (max: {self.MAX_WARNINGS})"
            )

        if len(self.validation_suggestions) > self.MAX_SUGGESTIONS:
            errors.append(
                f"Too many validation suggestions: {len(self.validation_suggestions)} (max: {self.MAX_SUGGESTIONS})"
            )
        ...
        for i, error in enumerate(self.validation_errors):
            if not isinstance(error, str):
                errors.append(
                    f"validation_errors[{i}] must be a string, got {type(error).__name__}"
                )

        for i, warning in enumerate(self.validation_warnings):
            if not isinstance(warning, str):
                errors.append(
                    f"validation_warnings[{i}] must be a string, got {type(warning).__name__}"
                )

        for i, suggestion in enumerate(self.validation_suggestions):
            if not isinstance(suggestion, str):
                errors.append(
                    f"validation_suggestions[{i}] must be a string, got {type(suggestion).__name__}"
                )
...
class MLComplianceResult(BaseValueObject):
    ...
    def validate(self) -> None:
        ...
        for field_name, score in score_fields:
            if score is not None:
                if not isinstance(score, int | float):
                    ...
                    self.MIN_COMPLIANCE_SCORE <= score <= self.MAX_COMPLIANCE_SCORE
                ):
                    errors.append(
                        f"{field_name} {score} must be between "
        ...
        if self.risk_level not in self.VALID_RISK_LEVELS:
            errors.append(
                f"risk_level must be one of {self.VALID_RISK_LEVELS}, got {self.risk_level}"
            )

        # Validate list sizes
        if len(self.policy_violations) > self.MAX_VIOLATIONS:
            errors.append(
                f"Too many policy violations: {len(self.policy_violations)} (max: {self.MAX_VIOLATIONS})"
            )

        if len(self.regulatory_flags) > self.MAX_FLAGS:
            errors.append(
                f"Too many regulatory flags: {len(self.regulatory_flags)} (max: {self.MAX_FLAGS})"
            )

        if len(self.checked_policies) > self.MAX_POLICIES:
            errors.append(
                f"Too many checked policies: {len(self.checked_policies)} (max: {self.MAX_POLICIES})"
            )

        if len(self.skipped_policies) > self.MAX_POLICIES:
            errors.append(
                f"Too many skipped policies: {len(self.skipped_policies)} (max: {self.MAX_POLICIES})"
            )
        ...
        for i, violation in enumerate(self.policy_violations):
            if not isinstance(violation, dict):
                errors.append(
                    f"policy_violations[{i}] must be a dict, got {type(violation).__name__}"
                )
                continue
            ...
            for field_name in required_fields:
                if field_name not in violation:
                    errors.append(
                        f"policy_violations[{i}] missing required field: {field_name}"
                    )
        ...
        for i, flag in enumerate(self.regulatory_flags):
            if not isinstance(flag, str):
                errors.append(
                    f"regulatory_flags[{i}] must be a string, got {type(flag).__name__}"
                )

        for i, policy in enumerate(self.checked_policies):
            if not isinstance(policy, str):
                errors.append(
                    f"checked_policies[{i}] must be a string, got {type(policy).__name__}"
                )

        for i, policy in enumerate(self.skipped_policies):
            if not isinstance(policy, str):
                errors.append(
                    f"skipped_policies[{i}] must be a string, got {type(policy).__name__}"
                )
    ...
    def validate_field(self, field_name: str, field_value: Any) -> None:
        ...
        if field_name in (
            ...
            if field_value is not None:
                ...
                if not (
                    self.MIN_COMPLIANCE_SCORE
                    <= field_value
                    <= self.MAX_COMPLIANCE_SCORE
                ):
                    raise ValueObjectValidationError(
                        f"{field_name} {field_value} must be between "
                        f"{self.MIN_COMPLIANCE_SCORE} and {self.MAX_COMPLIANCE_SCORE}"
    ...
    def requires_human_review(self) -> bool:
        ...
        return (
            ...
            or self.compliance_score < 0.8
            or len(self.policy_violations) > 0
        )
    ...
    def add_violation(
        self, policy_id: str, violation_type: str, severity: str, description: str
    ) -> "MLComplianceResult":
        """
