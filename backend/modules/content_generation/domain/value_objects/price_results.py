...
class ValidationError(BaseValueObject):
    ...
    def validate(self) -> None:
        ...
        if self.error_type not in self.VALID_ERROR_TYPES:
            errors.append(f"error_type must be one of {self.VALID_ERROR_TYPES}, got {self.error_type}")
        
        # Validate field name if provided
        if self.field_name is not None:
            if not isinstance(self.field_name, str):
                errors.append(f"field_name must be a string, got {type(self.field_name).__name__}")
...
class TitleValidationResult(BaseValueObject):
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.character_count, int):
            errors.append(f"character_count must be an integer, got {type(self.character_count).__name__}")
        elif self.character_count < 0:
            errors.append(f"character_count must be non-negative, got {self.character_count}")
        
        if not isinstance(self.word_count, int):
            errors.append(f"word_count must be an integer, got {type(self.word_count).__name__}")
        ...
        for list_name, error_list in error_lists:
            if len(error_list) > self.MAX_ERRORS:
                errors.append(f"Too many {list_name}: {len(error_list)} (max: {self.MAX_ERRORS})")
        ...
        if len(self.detected_keywords) > self.MAX_KEYWORDS:
            errors.append(f"Too many detected keywords: {len(self.detected_keywords)} (max: {self.MAX_KEYWORDS})")
        
        for i, keyword in enumerate(self.detected_keywords):
            if not isinstance(keyword, str):
                errors.append(f"detected_keywords[{i}] must be a string, got {type(keyword).__name__}")
        
        # Validate engine
        if not isinstance(self.validation_engine, str) or not self.validation_engine.strip():
        ...
        if self.character_count < self.MIN_TITLE_LENGTH and self.valid:
            errors.append(f"Valid title should be at least {self.MIN_TITLE_LENGTH} characters")
    ...
    def is_production_ready(self) -> bool:
        ...
        return (
            ...
            self.character_count >= self.MIN_TITLE_LENGTH
        )
...
class PriceEstimationResult(BaseValueObject):
    ...
    def validate(self) -> None:
        ...
        for field_name, price in price_fields:
            if not isinstance(price, Decimal):
                errors.append(f"{field_name} must be a Decimal, got {type(price).__name__}")
        ...
        if self.currency not in self.VALID_CURRENCIES:
            errors.append(f"currency must be one of {self.VALID_CURRENCIES}, got {self.currency}")
        ...
        if self.market_trend not in self.VALID_TRENDS:
            errors.append(f"market_trend must be one of {self.VALID_TRENDS}, got {self.market_trend}")
        ...
        for field_name, multiplier in multiplier_fields:
            if not isinstance(multiplier, (int, float)):
                errors.append(f"{field_name} must be a number, got {type(multiplier).__name__}")
            elif multiplier < 0:
        ...
        if len(self.pricing_factors) > self.MAX_FACTORS:
            errors.append(f"Too many pricing factors: {len(self.pricing_factors)} (max: {self.MAX_FACTORS})")
        ...
        for factor, weight in self.pricing_factors.items():
            ...
            if not (-1.0 <= weight <= 1.0):
                errors.append(f"pricing factor weight {weight} must be between -1.0 and 1.0")
        
        # Validate data sources
        if len(self.data_sources) > self.MAX_DATA_SOURCES:
            errors.append(f"Too many data sources: {len(self.data_sources)} (max: {self.MAX_DATA_SOURCES})")
        
        for i, source in enumerate(self.data_sources):
            if not isinstance(source, str):
                errors.append(f"data_sources[{i}] must be a string, got {type(source).__name__}")
        
        # Validate errors list
        if len(self.price_validation_errors) > self.MAX_ERRORS:
            errors.append(f"Too many validation errors: {len(self.price_validation_errors)} (max: {self.MAX_ERRORS})")
        ...
        if self.title_validation is not None and not isinstance(self.title_validation, TitleValidationResult):
            errors.append(f"title_validation must be a TitleValidationResult, got {type(self.title_validation).__name__}")
        
        # Validate confidence intervals
        if self.confidence_interval_95 is not None:
            if not isinstance(self.confidence_interval_95, tuple) or len(self.confidence_interval_95) != 2:
                ...
                if not isinstance(lower, Decimal) or not isinstance(upper, Decimal):
                    ...
                    errors.append("confidence_interval_95 lower bound must be less than upper bound")
        
        if self.confidence_interval_68 is not None:
            if not isinstance(self.confidence_interval_68, tuple) or len(self.confidence_interval_68) != 2:
                ...
                if not isinstance(lower, Decimal) or not isinstance(upper, Decimal):
                    ...
                    errors.append("confidence_interval_68 lower bound must be less than upper bound")
        ...
        if self.market_average is not None:
            if self.market_average < self.min_price or self.market_average > self.max_price:
                errors.append("market_average should be within min_price and max_price range")
    ...
    def estimation_quality(self) -> str:
        ...
        if (
            self.has_high_confidence and 
            self.similar_products_analyzed >= 5 and 
            not self.has_validation_errors
        ):
            return "high"
        elif (
            self.confidence_score >= 0.6 and 
            self.similar_products_analyzed >= 3
        ):
            return "medium"
        else:
            return "low"
    ...
    def requires_manual_review(self) -> bool:
        ...
        return (
            not self.has_high_confidence or
            self.has_validation_errors or
            self.similar_products_analyzed < 3 or
            self.price_range_percentage > 50
        )
