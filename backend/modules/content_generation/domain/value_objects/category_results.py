...
class CategoryAttribute(BaseValueObject):
    ...
    def validate(self) -> None:
        ...
        if self.attribute_type not in self.VALID_TYPES:
            errors.append(
                f"attribute_type must be one of {self.VALID_TYPES}, got {self.attribute_type}"
            )
        ...
        if not isinstance(self.importance_weight, int | float):
            errors.append(
                f"importance_weight must be a number, got {type(self.importance_weight).__name__}"
            )
        ...
        if self.attribute_type == "numeric" and self.value is not None:
            if not isinstance(self.value, int | float):
                errors.append(
                    f"numeric attribute value must be a number, got {type(self.value).__name__}"
                )
        elif self.attribute_type == "boolean" and self.value is not None:
            if not isinstance(self.value, bool):
                errors.append(
                    f"boolean attribute value must be a boolean, got {type(self.value).__name__}"
                )
        elif self.attribute_type == "text" and self.value is not None:
            if not isinstance(self.value, str):
                errors.append(
                    f"text attribute value must be a string, got {type(self.value).__name__}"
                )
        ...
        if (
            ...
            or not self.extraction_method.strip()
        ):
...
class CategoryAttributes(BaseValueObject):
    ...
    def __post_init__(self):
        ...
        if self.attributes:
            ...
            object.__setattr__(
                self, "average_importance", sum(importances) / len(importances)
            )
            object.__setattr__(
                self,
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.attributes, list):
            errors.append(
                f"attributes must be a list, got {type(self.attributes).__name__}"
            )
        ...
        if len(self.attributes) > self.MAX_ATTRIBUTES:
            errors.append(
                f"Too many attributes: {len(self.attributes)} (max: {self.MAX_ATTRIBUTES})"
            )
        ...
        for i, attribute in enumerate(self.attributes):
            if not isinstance(attribute, CategoryAttribute):
                errors.append(
                    f"attributes[{i}] must be a CategoryAttribute, got {type(attribute).__name__}"
                )

        # Validate extraction source
        if (
            not isinstance(self.extraction_source, str)
            or not self.extraction_source.strip()
        ):
        ...
        if self.total_attributes != len(self.attributes):
            errors.append(
                f"total_attributes {self.total_attributes} doesn't match actual count {len(self.attributes)}"
            )

        if self.attributes:
            actual_high_confidence = sum(
                1 for attr in self.attributes if attr.is_high_confidence
            )
    ...
    def get_top_attributes(self, count: int = 10) -> list[CategoryAttribute]:
        ...
        return sorted(
            self.attributes, key=lambda attr: attr.weighted_confidence, reverse=True
        )[:count]
    ...
    def validate(self) -> None:
        ...
        for i, attr in enumerate(self.allowed_attributes):
            if not isinstance(attr, str):
                errors.append(
                    f"allowed_attributes[{i}] must be a string, got {type(attr).__name__}"
                )

        for i, attr in enumerate(self.required_attributes):
            if not isinstance(attr, str):
                errors.append(
                    f"required_attributes[{i}] must be a string, got {type(attr).__name__}"
                )
        ...
        if not isinstance(self.external_mappings, dict):
            errors.append(
                f"external_mappings must be a dict, got {type(self.external_mappings).__name__}"
            )
        else:
            for key, value in self.external_mappings.items():
                if not isinstance(key, str) or not isinstance(value, str):
                    errors.append("external_mappings must have string keys and values")
                    break
    ...
    def validate_field(self, field_name: str, field_value: Any) -> None:
        ...
        if field_name in ("category_id", "category_name"):
            ...
            if not isinstance(field_value, int):
                raise ValueObjectValidationError(
                    f"level must be an integer, got {type(field_value).__name__}"
                )
            if field_value < 0:
                raise ValueObjectValidationError(
                    f"level must be non-negative, got {field_value}"
                )
            if field_value > self.MAX_LEVEL:
                raise ValueObjectValidationError(
                    f"level {field_value} exceeds maximum {self.MAX_LEVEL}"
                )

        elif field_name == "is_leaf_category":
            if not isinstance(field_value, bool):
                raise ValueObjectValidationError(
                    f"is_leaf_category must be a boolean, got {type(field_value).__name__}"
                )
...
class CategoryPredictionResult(BaseValueObject):
    ...
    alternative_predictions: list[tuple[CategoryInfo, float]] = field(
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.predicted_category, CategoryInfo):
            errors.append(
                f"predicted_category must be a CategoryInfo, got {type(self.predicted_category).__name__}"
            )
        ...
        for feature, importance in self.feature_importance.items():
            ...
            if not (0.0 <= importance <= 1.0):
                errors.append(
                    f"feature importance {importance} must be between 0.0 and 1.0"
                )

        # Validate analyzed attributes
        if not isinstance(self.analyzed_attributes, CategoryAttributes):
            errors.append(
                f"analyzed_attributes must be a CategoryAttributes, got {type(self.analyzed_attributes).__name__}"
            )

        # Validate prediction quality
        if self.prediction_quality not in self.QUALITY_LEVELS:
            errors.append(
                f"prediction_quality must be one of {self.QUALITY_LEVELS}, got {self.prediction_quality}"
            )
        ...
        if self.processing_time_ms is not None:
            if not isinstance(self.processing_time_ms, int):
                errors.append(
                    f"processing_time_ms must be an integer, got {type(self.processing_time_ms).__name__}"
                )
        ...
        if (
            not isinstance(self.prediction_method, str)
            or not self.prediction_method.strip()
        ):
    ...
    def is_confident_prediction(self) -> bool:
        ...
        return (
            ...
            and self.confidence_gap >= 0.2
            and self.prediction_quality in ("high", "medium")
        )
