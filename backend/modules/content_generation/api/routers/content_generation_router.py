...
# Dependency injection (these would be properly configured in production)
async def get_generate_content_use_case() -> GenerateContentUseCase:
    """Get the generate content use case instance."""
    # This would be properly dependency injected in production
    # For now, we'll return a mock instance
    raise HTTPException(
        status_code=status.HTTP_501_NOT_IMPLEMENTED,
        detail="Content generation use case not yet configured",
    )


async def get_current_user(_token: str = Depends(security)):
    """Get the current authenticated user."""
    # This would implement proper authentication in production
...
async def get_generated_content(
    ...
    try:
        # This would use the use case to get content
        # For now, return a mock response
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail="Get generated content not yet implemented",
        )
...
@router.websocket("/ws/processing/{processing_id}")
            ...
            await websocket_manager.broadcast_to_processing(
                ...
                ProgressUpdateMessage(
                    ...
                    data={
                        ...
                        "estimated_remaining_seconds": max(0, (100 - progress) * 0.3),
                    },
                ).model_dump(),
            )
