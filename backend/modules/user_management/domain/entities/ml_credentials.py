...
class MLCredentials:
    ...
    ml_auth_domain: Optional[str] = None  # Country-specific auth domain used
    ...
    def __post_init__(self) -> None:
        ...
        if self.ml_site_id not in valid_sites:
            raise ValueError(f"Invalid site_id: {self.ml_site_id}. Must be one of: {valid_sites}")
        ...
        if not required_scopes.issubset(current_scopes):
            raise ValueError(f"Missing required scopes. Required: {required_scopes}, Current: {current_scopes}")
    ...
    def should_refresh_token(self) -> bool:
        ...
        refresh_threshold = self.ml_expires_at.timestamp() - (5.5 * 3600)  # 5.5 hours before expiry
    ...
    def connection_health(self) -> str:
        ...
        if not self.ml_is_valid:
            return "invalid"
        elif self.is_refresh_token_expired:
            return "expired"
