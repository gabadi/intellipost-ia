...
class CredentialEncryptionService:
    ...
    def __init__(self, encryption_key: Optional[str] = None) -> None:
        ...
        Args:
            encryption_key: Base encryption key. If None, uses settings configuration.
        """
        if encryption_key:
            self._encryption_key = encryption_key
        else:
            # Get from settings, fallback to environment variable for backward compatibility
            from infrastructure.config.settings import settings
            self._encryption_key = settings.ml_encryption_key or os.getenv("ML_ENCRYPTION_KEY")
        
        if not self._encryption_key:
            # Get environment for validation
            from infrastructure.config.settings import settings
            if settings.is_production:
                raise ValueError("ML encryption key is required in production (set ML_ENCRYPTION_KEY or configure settings)")
            else:
                # Use test/development default
                self._encryption_key = "test-encryption-key-for-development-and-testing-only-32chars"
