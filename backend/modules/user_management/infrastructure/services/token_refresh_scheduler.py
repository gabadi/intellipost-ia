...
class TokenRefreshScheduler:
    ...
    async def _refresh_expiring_tokens(self) -> None:
        ...
        try:
            ...
            logger.info(f"Found {len(expiring_credentials)} credentials that need token refresh")
            ...
            for credentials in expiring_credentials:
                try:
                    if await self._should_refresh_token(credentials):
                        ...
                        logger.debug(f"Skipping refresh for credential {credentials.id}")
                        
                except Exception as e:
                    failed_count += 1
                    logger.error(f"Failed to refresh token for credential {credentials.id}: {e}")
                    ...
                    try:
                        ...
                        logger.error(f"Failed to mark credential as invalid: {save_error}")
    ...
    async def _should_refresh_token(self, credentials: MLCredentials) -> bool:
        ...
        if not credentials.ml_is_valid:
            logger.debug(f"Credential {credentials.id} is marked as invalid, skipping refresh")
            return False
    ...
    async def _refresh_single_token(self, credentials: MLCredentials) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to refresh tokens for credential {credentials.id}: {e}")
            raise
    ...
    async def get_refresh_status(self) -> dict:
        ...
        try:
            ...
            expiring_soon = await self._credentials_repository.find_expiring_tokens(soon_threshold)
            
            # Find invalid credentials
            invalid_credentials = await self._credentials_repository.find_invalid_credentials()
            
            return {
                "scheduler_running": self._running,
                "next_check_in_seconds": self._refresh_interval if self._running else None,
    ...
    async def force_refresh_credential(self, credential_id: str) -> bool:
        ...
        try:
            ...
            credentials = await self._credentials_repository.find_by_id(UUID(credential_id))
            if not credentials:
