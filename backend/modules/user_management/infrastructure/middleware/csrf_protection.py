...
class CSRFProtection:
    ...
    def __init__(self, secret_key: Optional[str] = None, expiry_seconds: int = 300):
        """
    ...
    def validate_state_token(self, token: str, user_id: str, additional_data: str = "") -> bool:
        """
        ...
        try:
            ...
            # Validate signature
            expected_payload = f"{token_user_id}:{timestamp_str}:{token_additional_data}"
    ...
    def validate_request_state(self, request: Request, user_id: str) -> None:
        ...
        if hasattr(request, 'json') and request.headers.get('content-type') == 'application/json':
            try:
                body = request.json()
