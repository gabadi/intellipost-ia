...
class MLOAuthInitiateResponse(BaseModel):
    ...
    expires_in: int = Field(
        default=300,
        description="Seconds until authorization expires"
    )
...
class MLOAuthCallbackResponse(BaseModel):
    ...
    ml_nickname: Optional[str] = Field(None, description="MercadoLibre user nickname")
    ml_email: Optional[str] = Field(None, description="MercadoLibre user email")
...
class MLConnectionStatusResponse(BaseModel):
    ...
    is_connected: bool = Field(..., description="Whether user is connected to MercadoLibre")
    ...
    ml_nickname: Optional[str] = Field(None, description="MercadoLibre user nickname")
    ml_email: Optional[str] = Field(None, description="MercadoLibre user email")
    ml_site_id: Optional[str] = Field(None, description="MercadoLibre site ID")
    expires_at: Optional[datetime] = Field(None, description="Token expiration time")
    last_validated_at: Optional[datetime] = Field(None, description="Last validation time")
    error_message: Optional[str] = Field(None, description="Error message if any")
    should_refresh: bool = Field(
        default=False,
        description="Whether token should be refreshed soon"
    )
    time_until_refresh: Optional[int] = Field(
        None,
        description="Seconds until token refresh is needed"
    )
...
class MLDisconnectRequest(BaseModel):
    ...
    confirm: bool = Field(
        default=True,
        description="Confirmation of disconnect action"
    )
